...
test('keeps only the three production entry points and fail-closed quality requirements', async () => {
  ...
  assert.match(common, /Collect required Gateway coverage[\s\S]*?dotnet-coverage collect/);
  assert.doesNotMatch(common, /continue-on-error: true/);
  assert.match(common, /Install required XML validator[\s\S]*?libxml2-utils/);
  assert.match(common, /Validate required coverage report[\s\S]*?validate-coverage-report\.sh/);
  ...
  assert.match(sonar, /COV_REPORT_PATH_UNSET/);
  assert.match(sonar, /sonar\.cs\.vscoveragexml\.reportsPaths=\$\{COVERAGE_FILE\}/);
