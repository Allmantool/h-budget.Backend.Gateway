...
> Coverage policy: test execution, positive test-count evidence, and coverage collection/validation/import are mandatory. A missing, empty, malformed, incompatible, or unvalidated report fails before scanning; no empty coverage variable or coverage-less analysis is permitted. Low measured coverage remains advisory to the repository-owned evaluator. The scanner does not wait for Sonar's native gate; `sonar-noncoverage-policy.mjs` waits for the submitted compute task and evaluates that task's analysis ID. For PRs, it validates the selected source SHA plus PR/source/base identity against Sonar's PR record; for branches, it validates the exact analysis-history revision. It accepts only `coverage` or `new_coverage` rejections. The native Sonar dashboard may remain red for coverage.
